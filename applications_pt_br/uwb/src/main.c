...
gint maincontrol()
{
  ...
  if (fcntl(fd, F_SETLK, &lock) < 0)
  {
    ...
    case EACCES:
      ...
      printf("Não! pid:%ld encontrei pid:%ld arquivo de bloqueio  %s\n", (long) getpid(), (long) lock.l_pid, wbconfig);
      return 1;
    }
    perror("Chamada da funcao fcntl falhou");
    return 1;
  }
  printf("Processo pid: %ld fez o bloqueio do arquivo com sucesso!\n", (long) getpid());
  return 0;
}
...
gint callpipe(gchar * command)
{
  ...
  THISFUNCBECALLED;
  debug("O processo de pesquisa é: \"%s\"\n", command);
  memset(buf, '\0', 256);
...
void call_shell_for_btEnv(char *shellfile)
{
  ...
  if (pid != 0)
  {
    ...
    if (WIFEXITED(stat_val))
      debug("filho saiu com sucesso\n");
    else
      printf("filho terminou com sucesso\n");
  }
  sleep(1);
  return;
}
...
// debug ("open_device success, and its handle: fd = 0x%x\n",fd); 
  if (fd == -1)
  {
    debug("Erro: %s:falha ao abrir arquivo \n",__func__); 
    g_iscardin = FALSE;
  }
  return (fd);
}
...
int tdsp_query_subsystem_id()
{
  int fd;
  gulong ret;
  debug("chamada da funcao: tdsp_query_subsystem_id \n");
  ret = 0;
  ...
  debug("Pesquisa de ID de barramento do subsistema: ret = 0x%lx\n", ret);
  close_device(fd);
  g_subsystemid = ret;
  return ret;
}
...
int tdsp_query_power_state()
{
  ...
  if (fd == -1)
  {
    debug(": falha na abertura do arquivo!\n");
    perror("Erro de abertura");
    return TDSP_SERIAL_NO_NOCARD;
  }
...
{
  debug("chamada da funcao: set_hotkey_flag(%ld)!\n", nmode);
  int fd;
...
{
  debug("chamada da funcao: set_antenna_flag(%ld)!\n", nmode);
  int fd;
  unsigned long mode = nmode;
  fd = open_device(TDSP_BUS_DEVICE_NAME);
  if (fd == -1)
...
{
  debug("chamada da funcao: tdsp_write_enum_mode(%ld)!\n", mode);
  int fd;
  fd = open_device(TDSP_BUS_DEVICE_NAME);
  if (fd == -1)
  {
...
{
  debug("chamada da funcao: initial_wb_defaultmode \n");
  int nenummode;
...
void tdsp_init_menu()
{
  debug("chamada da funcao: tdsp_init_menu()\n");
  tdsp_query_power_state();
  if (g_buspowerstate == PowerOn)
  {
...
{
  debug("chamada da funcao: adjust_menu_item_auto()\n");
  g_ismenuclicked = FALSE;
  gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(tdspuwbmenu.bluetooth), FALSE);
  gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(tdspuwbmenu.wlan), FALSE);
  gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(tdspuwbmenu.coexist), FALSE);
...
GtkWidget *create_pixmap_fromxpm(GtkWidget * window, char **string)
{
  ...
  debug("chamada da funcao: create_pixmap_fromxpm()\n");
...
void update_ver_string()
{
  ...
  {
    sprintf(chtmp, "driver de barramento: %s\n", "invalido");
  }
  strcat(chversion, chtmp);
...
void tdsp_refresh_version()
{
  ...
  debug("chamada da funcao: tdsp_refresh_version()\n");
  update_ver_string();
...
//  gtk_show_about_dialog(NULL, "version", VERSION,
  gtk_show_about_dialog(NULL, "versao", "",    //TDSPUWBVERION,
...
//    return 0;
 ...
 /**green 0, red 1, gray 2 **/
  if (g_iconindex != iconindex)
  {
    ...
    {
    case ICON_GREEN:
      show_notification(_("uWB Aviso"), _("3DSP esta inserido"), NULL, SWITCH_NOTIFY_TIME, NULL);
      break;
    case ICON_GRAY:
      show_notification(_("uWB Aviso"), _("3DSP nao esta inserido"), NULL, SWITCH_NOTIFY_TIME, NULL);
      break;
    default:
      break;
    }
...
gint main(gint argc, gchar * argv[])
{
  ...
  {
    g_print("Ja existe um processo rodando!\n");
    return 0;
  }
/***^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^***/
#ifdef ENABLE_NLS
